...
class Canada_Post_Shipping_For_WooCommerce_Shipping_Method extends WC_Shipping_Method {
	...
	private function check_for_products_without_weights() {
		...
		$products = wc_get_products($products_without_weights_args);
		
		//Find any product without a weight
		$all_products_have_weights = true;
		$product_without_weight = null;
		foreach ($products as $product) {
			if ($product->get_weight() == "") {
				$product_without_weight = $product;
				$all_products_have_weights = false;
			}

			if (!$all_products_have_weights) {
				break;
			}
		}
		...
		if (!$all_products_have_weights) {
			...
			$result .= '<br>For Example: <a href="' . $product->get_permalink() . '">' . $product->get_name() . ' - ' . $product->get_permalink() . '</a>';
